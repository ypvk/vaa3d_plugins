...
void Dialog::onOkButtonClicked()
{
 ...
 //get all the neuronTree data 
  ...
  //plot value
  /*
  vector<EdgePoint>& edgePoints = neuronTracing.get_edgePoints();
  unsigned char* data_r = image->getRawDataAtChannel(0);
  for(i = 0; i < edgePoints.size(); ++i)
  {
    plot(data_r, image->getXDim(), image->getYDim(), image->getZDim(),
        edgePoints[i].first, edgePoints[i].second, 255);
  }
  //v3dhandle newImage = m_callback->newImageWindow("tmp");
  m_callback->updateImageWindow(image_window, true); 
  // end plot value 
  */
  //plot the origin r info of the neuron swc
  NeuronTree neuronTree= m_traced_neurons[0];
  unsigned char* data_r = image->getRawDataAtChannel(0);
  QList <NeuronSWC>& neurnSWCs = neuronTree.listNeuron;
  for(i = 0; i < neurnSWCs.size(); i++)
  {
    NeuronSWC& unit = neurnSWCs[i];
    data_r[(int)unit.x + (int)unit.y*(image->getXDim()) + (int)unit.z*((image->getXDim())*(image->getYDim()))]=255;
  } 
  m_callback->updateImageWindow(image_window, true); 
  //end the plot
